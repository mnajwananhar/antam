...
export default auth((req: NextRequest & { auth?: any }) => {
  ...
  if (isLoggedIn && (nextUrl.pathname === "/" || nextUrl.pathname === "/auth/signin")) {
  ...
  if (nextUrl.pathname.startsWith("/input/") && isLoggedIn) {
    const userRole = req.auth?.user?.role
    const departmentId = req.auth?.user?.departmentId
    ...
    if (userRole === UserRole.PLANNER && routeDepartmentSlug) {
      ...
      const routeDepartmentName = departmentUtils.slugToName(routeDepartmentSlug)
