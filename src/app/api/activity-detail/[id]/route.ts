...
export async function PUT(
  ...
  try {
    ...
    if (
      existingActivity.operationalReport.isComplete &&
      session.user.role !== "ADMIN"
    ) {
      return NextResponse.json(
        {
          error:
            "Cannot edit activity in completed report. Admin approval required.",
        },
        { status: 403 }
      );
    }
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Cannot delete activity in completed report. Admin approval required.",
        },
        { status: 403 }
      );
    }
